...
def get_my_team_roster(file_path):
    ...
    print("Players extracted from my_team.md:", players)
    return players
...
def optimize_lineup():
    print("Loading data for lineup optimization...")
    
    # Load roster settings
    roster_settings = CONFIG.get('roster_settings', {})
    print("Roster Settings from config.yaml:", roster_settings)
    if not roster_settings:
        print("Error: Roster settings not found in config.yaml.")
        return
    ...
    try:
        ...
        print(f"Error: {PLAYER_PROJECTIONS_PATH} not found. Please run 'task download_player_data' first.")
        return
    ...
    if missing_players:
        print(f"Warning: Projections not found for: {missing_players}. Adding with placeholder points.")
    ...
    if my_team_projections.empty:
        print("No projections found for players on your team. Ensure player names match.")
        return
    ...
    if prob.status == 1: # LpStatus.Optimal
        ...
        print("No optimal solution found. The problem is infeasible. Check your roster and league settings.")
    else:
        print(f"Solver status: {LpStatus[prob.status]}. No optimal solution found.")
